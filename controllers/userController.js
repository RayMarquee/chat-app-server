...
exports.login = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: user._id }, process.env.JWT_SIGNATURE, { expiresIn: '7d' })
        res.json({ token })
    } catch (error) {
...
exports.register = async (req, res) => {
    try {
        ...
        await newUser.save()
        const token = jwt.sign({ userId: newUser._id }, process.env.JWT_SIGNATURE, { expiresIn: '7d' })
        res.json({ token })
    } catch (error) {
        console.log(error)
        res.json({ field: 'username', message: 'Server error' })
